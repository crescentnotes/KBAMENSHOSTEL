...
import { generateGatePassPDF, approveGatepass } from '../../services/pdfservice.js';
import { saveGatepass, getPendingGatepasses, updateGatepassStatus, getGatepassById, getAllGatepassRequests } from '../../db/dbservice.js';
import { sendGatepassEmail } from '../../services/mailservice.js';
import { approveGatePass, getGatePassDetails } from '../../controllers/gatepassController.js';
import { isRTAdminAuthenticated  } from '../../middlewares/authMiddleware.js';  // Import the authentication middleware

const router = express.Router();

// // RT Admin Route with authentication check
...
router.post('/approve-gatepass/:id', async (req, res) => {
    ...
    try {
        ...
        const loggedInUserEmail = req.session.user.email;  // Ensure to use the correct session key for user
